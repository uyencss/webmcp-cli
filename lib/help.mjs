...
Usage:
  ...
  ${PREVIEW_COMMAND} jev <command> [options]
  ...
  ${PREVIEW_COMMAND} project session plan|apply|list|prune|archive --target <project> [options]
  ${PREVIEW_COMMAND} project policy plan|apply [--workspace <dir>|--all] [--yes] [--json]
  ${PREVIEW_COMMAND} project <legacy> [options]   Legacy project surface: new --template, attach/list/where/doctor/charter/guide/schedule/content/policy/init/init-store/build-index/export-pack (Automation Runner)
...
Preview routing:
  ...
  project splits in the CLI: legacy subcommands route directly to the
  Automation Runner (no Browser leg); the new archetype/plugin/inspect/repair/
  session routes delegate to the Project Kit executable.
  workflow/ai/jev/site/automation/vault/mobile/adb/captcha delegate to explicit
...
Environment:
  ...
  WEBMCP_JEV_BIN                Override Jev decision runtime path or package name
