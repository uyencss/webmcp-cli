...
Usage:
  ...
  ${PREVIEW_COMMAND} project <command> [options]
  ${PREVIEW_COMMAND} project-kit <command> [options]   Optional; needs an installed Project Kit CLI
...
Preview routing:
  ...
  delegate to the Browser executable; doctor/bootstrap/project/skills are
  explicit legacy adapters to the same Browser executable.
  ...
  project-kit delegates only to an explicit Project Kit executable when one is
  installed; it never replaces the legacy project route.
