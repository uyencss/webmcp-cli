...
function readSkillInventory() {
  ...
  const candidates = explicit
    ...
    : [
        resolve(PACKAGE_ROOT, '..', '..', 'webmcp-kit.json'), // checkout-local kit root
        resolve(PACKAGE_ROOT, '..', 'webmcp-router', 'webmcp-kit.json'), // release payload layout
  ...
  for (const file of candidates) {
    ...
    try {
      ...
      const payloadRoot = basename(dirname(file)) === 'webmcp-router' ? dirname(dirname(file)) : undefined;
      const payloadFields = payloadRoot ? { payloadRoot } : {};
      if (data.schema === 'webmcp-kit/1' && Array.isArray(data.skills)) {
        return { file, root: dirname(file), kitId: data.kitId ?? 'webmcp-automation-kit', skills: data.skills, superseded, ...payloadFields };
      }
      if (data.schema === 'webmcp-skill-catalog/1' && Array.isArray(data.skills)) {
        return { file, root: resolve(dirname(file), '..'), skills: data.skills, superseded, ...payloadFields };
      }
    } catch {
...
function skillPath(inventory, skill) {
  ...
  if (inventory.payloadRoot) {
    const source = skill.source || '';
    if (source.startsWith('packages/') || source.startsWith('stores/')) {
      const rest = source.split('/').slice(1).join('/');
      if (rest) {
        const remapped = resolve(inventory.payloadRoot, rest);
        if (existsSync(remapped)) return remapped;
      }
    } else if (source.startsWith('skills/')) {
      const remapped = resolve(inventory.payloadRoot, 'webmcp-router', source);
      if (existsSync(remapped)) return remapped;
    }
  }
