...
import { delegateCapture } from '../delegate.mjs';
...
async function collectBrowserSection() {
  ...
  if (result.timedOut || result.spawnErrorMessage) {
    return {
      status: 'error',
...
export async function collectCliDoctorReport() {
  ...
  const browser = await collectBrowserSection();
