...
export const COMPONENTS = {
  ...
  jev: {
    label: 'WebMCP Jev decision runtime',
    installHint: 'Install @gyga-browser/webmcp-ai with the webmcp-jev bin, run from the webmcp-automation-kit checkout, or set WEBMCP_JEV_BIN.',
    envVars: ['WEBMCP_JEV_BIN'],
    overrideSubpaths: ['bin/webmcp-jev.mjs'],
    siblings: ['../webmcp-ai-cli/bin/webmcp-jev.mjs'],
    packages: [
...
export const INSTALLED_COMPONENTS = {
  ...
  jev: { componentId: 'webmcp-ai-cli', bins: ['webmcp-jev'] },
...
export const RUNNER_SIBLINGS = ['../webmcp-automation-runner/bin/webmcp-automation-runner.mjs'];

export const RUNNER_PACKAGE = {
  name: '@gyga-browser/webmcp-automation-runner',
  subpaths: ['bin/webmcp-automation-runner.mjs'],
};
...
export function resolveRunnerBin(options = {}) {
  ...
  if (override) return resolveOverride(override, RUNNER_PACKAGE.subpaths, { cwd, packageRoot });
  if (isInstalledMode(env)) {
    return resolveFromInstalledManifest('runner', env, cwd);
  }
  for (const sibling of RUNNER_SIBLINGS) {
    const candidate = resolve(packageRoot, sibling);
    if (existsSync(candidate)) return candidate;
  }
  return resolvePackageSubpath(RUNNER_PACKAGE.name, RUNNER_PACKAGE.subpaths, packageRoot);
}
...
export function runnerNotFoundMessage(options = {}) {
  const devMessage = [
  ...
  if (state.installed && state.ok === false) {
    return [
      'WebMCP Automation Runner CLI not found (installed mode).',
      `Installed release manifest is invalid: ${state.manifestPath} (installed mode: no source/npm fallback).`,
    ].join('\n');
  }
  return [
    'WebMCP Automation Runner CLI not found (installed mode).',
    'The selected release does not contain component webmcp-automation-runner (installed mode: no source/npm fallback).',
  ].join('\n');
}
