...
async function runComponent(command, route, args, { env, runChild }) {
  if (route.component === 'mobile') {
    ...
    if (!adbBin || !existsSync(adbBin)) {
      console.error(componentNotFoundMessage('mobile'));
      return 1;
    }
  ...
  if (!bin || !existsSync(bin)) {
    console.error(componentNotFoundMessage(route.component));
    return 1;
  }
...
export async function main(argv = process.argv.slice(2), deps = {}) {
  ...
  if (route.kind === 'browser' || route.kind === 'legacy-browser') {
  ...
  if (route.kind === 'project-kit') {
    ...
    if (!kitBin || !existsSync(kitBin)) {
      console.error(projectKitNotFoundMessage());
      return 1;
    }
    return runChild({
